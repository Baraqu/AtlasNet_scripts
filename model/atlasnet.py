...
class Atlasnet(nn.Module):
    ...
    def generate_mesh(self, latent_vector):
        ...
        output_meshes = [ms.Mesh(vertices=output_points[i].transpose(1, 0).contiguous().cpu().numpy(),
                                          faces=self.template[i].mesh.faces)
                         for i in range(self.opt.nb_primitives)]
        #print([output_meshes[0].faces, output_meshes[0].vertices ])

        # Deform return the deformed pointcloud
        mesh = ms.merge_meshes(output_meshes)
